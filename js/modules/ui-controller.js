var UIController = {
    ...
    // Evita que texto escrito por usuarios (título, descripción, nombre) se interprete como HTML/JS
    // al insertarlo con innerHTML -- si no se escapa, alguien podría publicar un título con
    // <img onerror=...> y ejecutar código en el navegador de quien vea esa tarjeta.
    escHtml: function(str) {
        if (!str) return '';
        return String(str).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
    },
    renderizarTarjetaArticulo: function(art) {
        var tituloSeguro = this.escHtml(art.titulo), descSeguro = this.escHtml(art.descripcion), imgSeguro = this.escHtml(art.imagen_url);
        ...
        return '<div class="card">' + imagenHTML + '<h3 class="card-title">' + tituloSeguro + '</h3>' +
            ...
            '<span class="badge badge-disponible">🟢 ' + this.escHtml(art.modalidad) + '</span></div>' +
            '<button class="btn-read-more" onclick="UIController.abrirModal(\'' + tituloSeguro.replace(/'/g, "\\'") + '\',\'' + descSeguro.replace(/'/g, "\\'") + '\',\'' + imgSeguro.replace(/'/g, "\\'") + '\',\'' + (art.icono || '📦') + '\')">Ver detalles</button></div>';
    },
...
if (resultado.coincidencias === 0 && hayExterno) { html += '<div style="text-align:center;padding:20px;"><p>No tienes productos publicados para esto, pero encontramos lo siguiente:</p></div>'; } else if (resultado.coincidencias === 0) { html += '<div style="text-align:center;padding:40px;"><p>No encontramos artículos. Intenta con sinónimos (ej: "camisa" o "polo").</p></div>'; } else { html += '<div id="resultadosProductosLista"></div>'; } var self = this; if (resultado.resultados_web && resultado.resultados_web.length) { html += '<div class="ai-context-banner" style="margin-top:20px;">🌐 <strong>Resultados de internet</strong></div>'; html += resultado.resultados_web.map(function(w) { return '<div class="result-item" onclick="window.open(\'' + self.escHtml(w.link) + '\', \'_blank\')"><div class="result-icon-fallback">🌐</div><div class="result-info"><div class="result-title">' + self.escHtml(w.titulo) + '</div><div class="result-desc">' + self.escHtml(w.resumen || '') + '</div></div></div>'; }).join(''); } if (resultado.resultados_videos && resultado.resultados_videos.length) { html += '<div class="ai-context-banner" style="margin-top:20px;">🎬 <strong>Videos de YouTube</strong></div>'; html += resultado.resultados_videos.map(function(v) { var miniatura = v.miniatura ? '<img src="' + self.escHtml(v.miniatura) + '" class="result-img" alt="' + self.escHtml(v.titulo) + '">' : '<div class="result-icon-fallback">🎬</div>'; return '<div class="result-item" onclick="window.open(\'' + self.escHtml(v.link) + '\', \'_blank\')">' + miniatura + '<div class="result-info"><div class="result-title">' + self.escHtml(v.titulo) + '</div><div class="result-category">' + self.escHtml(v.canal) + '</div></div></div>'; }).join(''); } this.elementos.searchResultsContent.innerHTML = html; if (resultado.coincidencias > 0) { Paginador.inicializar('resultadosProductosLista', 'paginacionBusqueda', resultado.resultados, 9, this.renderizarItemResultado); } else { var pc = document.getElementById('paginacionBusqueda'); if (pc) pc.innerHTML = ''; } },
    ...
    mostrarResultadosVideo: function(tema, videos, etiqueta) {
        ...
        if (!videos.length) {
            ...
            html += videos.map(function(v) {
                var miniatura = v.miniatura ? '<img src="' + self.escHtml(v.miniatura) + '" class="result-img" alt="' + self.escHtml(v.titulo) + '">' : '<div class="result-icon-fallback">' + etiqueta.icono + '</div>';
                return '<div class="result-item" onclick="window.open(\'' + self.escHtml(v.link) + '\', \'_blank\')">' + miniatura + '<div class="result-info"><div class="result-title">' + self.escHtml(v.titulo) + '</div><div class="result-category">' + self.escHtml(v.canal) + '</div></div></div>';
            }).join('');
        }
    ...
    mostrarResultadosWeb: function(tema, resultadosWeb) {
        ...
        if (!resultadosWeb.length) {
            ...
            html += resultadosWeb.map(function(w) {
                return '<div class="result-item" onclick="window.open(\'' + self.escHtml(w.link) + '\', \'_blank\')"><div class="result-icon-fallback">🌐</div><div class="result-info"><div class="result-title">' + self.escHtml(w.titulo) + '</div><div class="result-desc">' + self.escHtml(w.resumen || '') + '</div></div></div>';
            }).join('');
        }
        this.elementos.searchResultsContent.innerHTML = html;
    },
    mostrarResultadosPersonas: function(nombreBuscado, usuarios, nivelZona) {
        ...
        if (!usuarios.length) {
            ...
            html += usuarios.map(function(u) {
                var nombreSeguro = self.escHtml(u.nombre_completo);
                var iniciales = (u.nombre_completo || 'U').trim().charAt(0).toUpperCase();
                var avatar = u.foto_perfil ? '<img src="' + self.escHtml(u.foto_perfil) + '" class="result-img" alt="' + nombreSeguro + '">' : '<div class="result-icon-fallback">' + iniciales + '</div>';
