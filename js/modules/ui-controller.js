var UIController = {
    ...
    renderizarTarjetaArticulo: function(art) {
        var imagenHTML = art.imagen_url ? '<img src="' + art.imagen_url + '" class="card-img-real" alt="' + art.titulo + '">' : '<div class="card-img-top">' + (art.icono || '') + '</div>';
        return '<div class="card">' + imagenHTML + '<h3 class="card-title">' + art.titulo + '</h3>' +
            '<p class="card-text">' + art.descripcion + '</p>' +
            '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;">' +
            '<span style="font-size:20px;font-weight:bold;color:var(--verde-esmeralda);">S/ ' + art.precio + '</span>' +
            '<span class="badge badge-disponible">🟢 ' + art.modalidad + '</span></div>' +
            '<button class="btn-read-more" onclick="UIController.abrirModal(\'' + art.titulo.replace(/'/g, "\\'") + '\',\'' + art.descripcion.replace(/'/g, "\\'") + '\',\'' + (art.imagen_url || '') + '\',\'' + (art.icono || '📦') + '\')">Ver detalles</button></div>';
    },
...
if (resultado.coincidencias === 0 && hayExterno) { html += '<div style="text-align:center;padding:20px;"><p>No tienes productos publicados para esto, pero encontramos lo siguiente:</p></div>'; } else if (resultado.coincidencias === 0) { html += '<div style="text-align:center;padding:40px;"><p>No encontramos artículos. Intenta con sinónimos (ej: "camisa" o "polo").</p></div>'; } else { html += '<div id="resultadosProductosLista"></div>'; } if (resultado.resultados_web && resultado.resultados_web.length) { html += '<div class="ai-context-banner" style="margin-top:20px;">🌐 <strong>Resultados de internet</strong></div>'; html += resultado.resultados_web.map(function(w) { return '<div class="result-item" onclick="window.open(\'' + w.link + '\', \'_blank\')"><div class="result-icon-fallback">🌐</div><div class="result-info"><div class="result-title">' + w.titulo + '</div><div class="result-desc">' + (w.resumen || '') + '</div></div></div>'; }).join(''); } if (resultado.resultados_videos && resultado.resultados_videos.length) { html += '<div class="ai-context-banner" style="margin-top:20px;">🎬 <strong>Videos de YouTube</strong></div>'; html += resultado.resultados_videos.map(function(v) { var miniatura = v.miniatura ? '<img src="' + v.miniatura + '" class="result-img" alt="' + v.titulo + '">' : '<div class="result-icon-fallback">🎬</div>'; return '<div class="result-item" onclick="window.open(\'' + v.link + '\', \'_blank\')">' + miniatura + '<div class="result-info"><div class="result-title">' + v.titulo + '</div><div class="result-category">' + v.canal + '</div></div></div>'; }).join(''); } this.elementos.searchResultsContent.innerHTML = html; if (resultado.coincidencias > 0) { Paginador.inicializar('resultadosProductosLista', 'paginacionBusqueda', resultado.resultados, 9, this.renderizarItemResultado); } else { var pc = document.getElementById('paginacionBusqueda'); if (pc) pc.innerHTML = ''; } },
    mostrarResultadosVideo: function(tema, videos) {
        ...
        var html = '<div class="ai-context-banner hibrido">🎬 <strong>Videos de YouTube</strong></div>';
        if (!videos.length) {
            html += '<div style="text-align:center;padding:40px;"><p>No encontramos videos sobre eso. Intenta con otras palabras.</p></div>';
        } else {
            html += videos.map(function(v) {
                var miniatura = v.miniatura ? '<img src="' + v.miniatura + '" class="result-img" alt="' + v.titulo + '">' : '<div class="result-icon-fallback">🎬</div>';
                return '<div class="result-item" onclick="window.open(\'' + v.link + '\', \'_blank\')">' + miniatura + '<div class="result-info"><div class="result-title">' + v.titulo + '</div><div class="result-category">' + v.canal + '</div></div></div>';
            }).join('');
        }
