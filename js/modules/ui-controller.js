...
var UIController = {
    ...
    renderizarItemResultado: function(art) { var icono = art.icono || ''; var tituloSeguro = escHtml(art.titulo), descSeguro = escHtml(art.descripcion), imgSeguro = escHtml(art.imagen_url), catSeguro = escHtml(art.categoria), paisSeguro = escHtml(art.pais), modSeguro = escHtml(art.modalidad); var imagenHTML = art.imagen_url ? '<img src="' + imgSeguro + '" class="result-img" alt="' + tituloSeguro + '">' : '<div class="result-icon-fallback">' + icono + '</div>'; var exp = art._es_externo ? '<span class="badge badge-externo">🌐 Referencia Global</span>' : (art._es_expandido ? '<span class="badge badge-expandido">🌍 Zona lejana</span>' : ''); var pais = art.pais ? '<span class="badge badge-pais">📍 ' + paisSeguro + '</span>' : ''; var modal = art.modalidad ? '<span class="badge badge-modalidad">' + modSeguro + '</span>' : ''; var distanciaHTML = (typeof art.distancia_km === 'number' && !isNaN(art.distancia_km)) ? '<span class="badge badge-distancia"> ' + art.distancia_km + ' km</span>' : ''; return '<div class="result-item" onclick="UIController.abrirModal(\'' + tituloSeguro.replace(/'/g, "\\'") + '\',\'' + descSeguro.replace(/'/g, "\\'") + '\',\'' + imgSeguro.replace(/'/g, "\\'") + '\',\'' + icono + '\')">' + imagenHTML + '<div class="result-info"><div class="result-title">' + tituloSeguro + '</div><div class="result-category">' + catSeguro + '</div><div class="result-desc">' + descSeguro.substring(0, 100) + '...</div>' + '<div class="result-badges"><span class="badge badge-disponible">🟢 Disponible</span>' + distanciaHTML + pais + modal + exp + '</div></div>' + '<div class="result-actions"><div class="result-price">S/ ' + art.precio + '</div><button class="btn-contactar" onclick="event.stopPropagation(); UIController.contactarProducto(' + (art.usuario_id ? "'" + art.usuario_id + "'" : 'null') + ', \'' + tituloSeguro.replace(/'/g, "\\'") + '\')">📞 Contactar</button></div></div>'; },
