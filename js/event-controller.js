var EventController = {
    ...
    procesarAccionIA: async function(datos, queryOriginal) {
        ...
        if (accion === 'BUSCAR') {
            ...
            // La IA misma avisó que no está segura de haber entendido -- antes de rendirnos,
            // se intenta igual la búsqueda literal de lo que la persona escribió (sea lo que
            // sea: un producto, una categoría, cualquier texto), con el mismo motor genérico
            // que ya usa toda búsqueda normal. Así, si la IA se confundió con el formato de
            // datos pero lo que la persona pidió sí existe en el catálogo, igual aparece.
            ...
            if (resultadoRespaldo.resultados && resultadoRespaldo.resultados.length) {
                ...
                // guía a la persona con ejemplos concretos de frases que sí funcionan, en vez
                // de mostrar un mensaje contradictorio al lado de una búsqueda que no llevó a nada.
